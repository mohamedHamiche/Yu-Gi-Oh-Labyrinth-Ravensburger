...
int main(int argc, char *argv[]){
	...
	window = SDL_CreateWindow("SDL2", SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED, 800,600,SDL_WINDOW_SHOWN);
	if(window == NULL){
	...
	SDL_SetRenderDrawColor(renderer,15,100,15,255);
	SDL_RenderClear(renderer); // pour la fenetre entiere

	TUILE plateau[7][7];
	tuilesFixes(plateau);

	char Nom[14] = "img/a.bmp";
	unsigned int suiv=0;
	for(int i=0 ;i<7; i+=2){
		for(int j=0; j<7; j+=2){
    		...
    		for(int j=0; j<7; j++){
    			if((j%2 == 0) && (i%2 == 0) ){
    				...
    				plateau[i][j].texture=SDL_CreateTextureFromSurface(renderer, imageTCtresor[r]);
                     SDL_FreeSurface(imageTCtresor[r]);
    			}
    		}
    		printf("\n");	
...
printf("\n");
    ...
    int positionX = (800 - 490)/2;
    int positionY = (600 -490)/2; 
    int j=0;
